...
public class abasnet_PA5_PartB 
{
	...
	private static void readFile(String fileName) throws IOException
	{
		...
		while (readFile.hasNextFloat()) 
		{
			...
			float value_Of_T = calculate_T(point_OnPlane, normal_ToPlane,  point_ToProj);
			Vector<Float> point_After_Projection = calculate_Point_AfterProj(point_ToProj, value_Of_T);
			...
			// new line after 3 points per line
			if (countLines > 2)
			{
				  output.write("\n");
				  countLines = 0;
	...
	//calculate projected point after perspective projection
	private static Vector<Float> calculate_Point_AfterProj(Vector<Float> point_ToProj, float value_Of_T)
	{
		Vector<Float> vector = new Vector<Float>();
		float tx1 = value_Of_T * point_ToProj.get(0);
		float tx2 = value_Of_T * point_ToProj.get(1);
		float tx3 = value_Of_T * point_ToProj.get(2);
		
		float x1_Prime = point_ToProj.get(0) + tx1;
		float x2_Prime = point_ToProj.get(1) + tx2;
		float x3_Prime = point_ToProj.get(2) + tx3;
	...
	}// calculate_Point_AfterProj

	// calculate value of t for perspective projection
	private static float calculate_T(Vector<Float> point_OnPlane, Vector<Float> normal_ToPlane,
			Vector<Float> point_ToProj) 
	{
		...
		float x_Dot_N1 = x1 * n1;
		float x_Dot_N2 = x2 * n2;
		float x_Dot_N3 = x3 * n3;

		float x_Dot_N = x_Dot_N1 + x_Dot_N2 + x_Dot_N3;

		t = q_Minus_X_Dot_N / x_Dot_N;
	...
	private static String outputFile() 
	{
		...
		{
		case 1:
			output_FileName = "abasnet_PA5_output_B_1.txt";
			break;

		case 2:
			output_FileName = "abasnet_PA5_output_B_2.txt";
			break;
